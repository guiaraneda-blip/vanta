...
def menu():
    while True:
        ...
        print(CYAN + "  " + "─" * 40 + RESET)
        print()
        print(f"  {CYAN}[0]{RESET}   {WHITE}Anonymity Mode{RESET}")
        ...
        print(f"  {CYAN}[6]{RESET}   {WHITE}Backup & Recovery{RESET}")
...
def legal_disclaimer():
    ...
    print(f"{CYAN}║{RESET}{'AVISO LEGAL — VANTA OS':^48}{CYAN}║{RESET}")
    print(f"{CYAN}{'═'*50}{RESET}")
    print()
    print_slow("  VANTA OS es una herramienta para", delay=0.02)
    print_slow("  profesionales de ciberseguridad.", delay=0.02)
    print()
    print_slow("  El uso de estas herramientas sin", delay=0.02)
    print_slow("  autorizacion expresa del objetivo", delay=0.02)
    print_slow("  es ILEGAL y responsabilidad", delay=0.02)
    print_slow("  exclusiva del usuario.", delay=0.02)
    print()
    print_slow("  Al continuar, aceptas usar VANTA OS", delay=0.02)
    print_slow("  solo en sistemas autorizados.", delay=0.02)
    print()
    print(f"{CYAN}{'═'*50}{RESET}")
    print()
    print(f"  {CYAN}[ENTER]{RESET} Acepto y continuo")
    print(f"  {GRAY}[q]     Salir{RESET}")
    print()
    choice = input(f"  {CYAN}» {RESET}").strip().lower()
    if choice == "q":
        print()
        print_slow("  Saliendo...", delay=0.04, color=GRAY)
        exit(0)
